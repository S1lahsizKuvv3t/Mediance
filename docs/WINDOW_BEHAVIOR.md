...
WinUI receives one initialization activation and immediately restores the previously foreground application. The main widget then uses `WS_EX_NOACTIVATE`, so pointer interaction does not take foreground ownership from a borderless game or another app. Both the widget and settings window use `WS_EX_TOOLWINDOW`, keeping them out of the taskbar and Alt+Tab; the settings window remains explicitly activatable when opened. Optional Windows startup passes `--background` and leaves the widget hidden in the tray. A named single-instance coordinator makes later launches reveal the existing widget without activation, then closes the secondary process. Closing awaits media cleanup before destroying the last window. Alt+F4 also follows the same close path.
